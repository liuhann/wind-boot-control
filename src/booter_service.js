...
const DBRestify = require('./db_restify')
const os = require('os')
...
module.exports = class BooterService {
    ...
    async initRoute (router) {
      ...
      // 获取日志文件列表
      router.get(this.routePrefix + '/log/list', async (ctx, next) => {
        ctx.body = ctx.app.services.logSerivce.getLogFiles()
        await next()
      })

      // 获取日志内容
      router.get(this.routePrefix + '/log/tail', async (ctx, next) => {
        const { file, lines } = ctx.request.query
        ctx.body = await readLastLine.read(path.resolve('./', file), lines || 500)
        await next()
      })

      // 获取DBList
      router.get(this.routePrefix + '/database/list', async (ctx, next) => {
        ctx.body = await this.app.getDbs()
        await next()
      })

      // 设置coll为restful
      router.get(this.routePrefix + '/database/collection/restful', async (ctx, next) => {
        const { db, coll } = ctx.request.query
        const ry = new DBRestify(db, coll)
        ctx.body = await ry.initRoute(this.app)
        await next()
      })

      // 获取DB Collection List
      router.get(this.routePrefix + '/database/:dbname/collections', async (ctx, next) => {
        const { dbname } = ctx.params
        const db = await this.app.getDb(dbname)
        ctx.body = await db.getCollections()
        await next()
      })
    ...
    async getHotModules () {
      return (await this.pkgColl.find({}))
    }
    ...
    async loadStartHotPackages () {
      const packs = await this.pkgColl.find({})
    ...
    async installHotModule (name) {
      ...
      const installedPackage = require(name)
